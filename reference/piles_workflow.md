...
If `outfile` is NA, returns an sf object (filtered or unfiltered based
on `filter_return`). If `outfile` is a character string, returns the
character string of the saved file path.
