...
A full demonstration script of the install and setup of `cloud2trees` including dependancies is available at: [https://github.com/georgewoolsey/cloud2trees/blob/main/inst/examples/example.R](https://github.com/georgewoolsey/cloud2trees/blob/main/inst/examples/example.R)

It is recommended that users follow this full install demonstration which includes program functionality checks after the intallation steps are completed. 

For users comfortable with installing and setting up packages in R, a quick setup demonstration follows below.
...
The `cloud2trees` package relies on external data to estimate tree DBH, FIA forest type, and LANDFIRE fuel loading. If you plan to use this functionality, the data must be downloaded first using `get_data()` which only needs to be run the first time you use the `cloud2trees` package. All downloads combine to use ~7 GB of storage space.
...
Unless you have already executed the `get_data()` function, you must first download the FIA data (~4 GB) using `get_treemap()` which only needs to be run the first time you use the `cloud2trees` package.
...
To estimate FIA forest type group for trees requires external data to spatially determine the forest type that of a tree.

Forest type groups are identified using the [Forest Type Groups of the Continental United States](https://www.arcgis.com/home/item.html?id=10760c83b9e44923bd3c18efdaa7319d) data (Wilson 2023). This forest type group layer was developed using data from over 213,000 national forest inventory plots measured during the period 2014-2018 from the FIA program. This raster layer has 30-meter resolution and covers the forested extent of the continental US.

Unless you have already executed the `get_data()` function, you must first download the FIA data (~2 GB) using `get_foresttype()` which only needs to be run the first time you use the `cloud2trees` package.
...
To estimate tree crown biomass for trees using [LANDFIRE's Forest Canopy Bulk Density (CBD)](https://landfire.gov/fuel/cbd) estimates requires external data to spatially determine the fuel loading of the area.

The `cloud2trees` package currently uses [LANDFIRE's Forest Canopy Bulk Density (CBD)](https://landfire.gov/fuel/cbd) 2023 estimates ("CONUS LF 2023") at 30-meter resolution over the entire continental US.

Unless you have already executed the `get_data()` function, you must first download the FIA data (~2.5 GB) using `get_landfire()` which only needs to be run the first time you use the `cloud2trees` package.
...
there is a plot of the different window size functions tested (plot columns) over the different 0.1 ha sample plots (plot rows) and the number of individual trees extracted shown outlined in gray overlaid on the canopy height model (CHM).
...
run `itd_tuning()` with our custom window size definitions and this time we'll only try on two sample plots of 0.1 ha
...
The astute reader will have noticed that we saved our outputs to a temporary directory (`my_dir`) but this could have been any directory on our local machine (e.g. "C:\\Data"). Let's check out the files that were delivered in the *point_cloud_processing_delivery* folder.
...
dplyr::tibble(
  file_name = c(
    "cbh_height_model_estimates.rds", "chm_0.25m_tif", "dtm_1m.tif", "fia_foresttype_raster.tif",
    "final_detected_crowns.gpkg", "final_detected_tree_tops.gpkg", "hmd_height_model_estimates.rds",
    "processed_tracking_data.csv", "raw_las_ctg_info.gpkg", "regional_dbh_height_model.rds",
    "regional_dbh_height_model_estimates.csv", "regional_dbh_height_model_predictions.csv",
    "regional_dbh_height_model_training_data.csv", "stand_cell_data_cruz.csv",
    "stand_cell_data_landfire.csv", "Cloud2Trees_TreeList.txt", "dtm_Clipped.tif", "fuellist",
    "Lidar_Bounds.geojson", "topo.dat"
  ),
  file_type = c(
    "R Data Serialization", "GeoTif", "GeoTif", "GeoTif", "Geopackage", "Geopackage",
    "R Data Serialization", "CSV", "Geopackage", "R Data Serialization", "CSV", "CSV", "CSV",
    "CSV", "CSV", "Text", "GeoTif", "File", "GeoJSON", "DAT File"
  ),
  description = c(
    "saved R object of random forest model predicting tree crown base height",
    "canopy height model derived at user-defined resolution",
    "digital terrain model derived at user-defined resolution",
    "30 m resolution raster of predicted forest type",
    "polygons of individual tree crowns with appended tree level attributes",
    "tree top point locations of individual trees with appened tree level attributes",
    "saved R object of random forest model predicting tree height to max crown diameter",
    "summary table of data processing time for each step from DTM/CHM generation through modeling tree parameters",
    "polygon of las tiles used in data processing",
    "saved R object of polynomial regression to predict tree diameter at breast height",
    "table of predicted coefficient values of polynomial regression to predict tree diameter at breast height",
    "predicted diameter at breast height from polynomial regression at 0.1 m height increments",
    "table of TreeMap tree data used to train diameter at breast height prediction model",
    "table of parameters used to predict CBD using Cruz et al. 2003 and to convert to tree level crown bulk density",
    "table of parameters used to convert LANDFIRE CBD to tree level crown bulk density",
    "tree list with formatted spacing containing tree parameters necessary for input to the LANL TREES program",
    "digital terrain model clipped to a projected rectangular extent and scaled to 2 m resolution for use in LANL TREES program",
    "file describing treelist file and surface fuel litter and herbaceous bulk density, depth, surface area to volume, and moisture as user defined constants for use in the LANL TREES program",
    "projected polygon delineating the data and simulation extent",
    "digital terrain model elevation data stored in FORTRAN format for use in LANL TREES program"
  )
) %>% 
kableExtra::kbl(
  caption = "Cloud2Trees Output File Descriptions"
  , col.names = c(
      "File Name"
      , "File Type"
      , "Description"
...
We developed the `cloud2trees_to_lanl_trees()` function to use the output from `cloud2trees()` to generate inputs for the [LANL TREES](https://github.com/lanl/Trees/) program as a pathway to fire modeling with [Quic-Fire](https://doi.org/10.1016/j.envsoft.2019.104616)
...
We'll pretend our area of interest is the central 2,000 m^2^ of our point cloud data
...
Now, we'll call the function and tell it where our `cloud2trees()` outputs are located and what our study area is. We'll also tell it just to write the inputs for the [LANL TREES](https://github.com/lanl/Trees/) program to the same directory and the program will automatically create a new folder titled *lanl_trees_delivery*
...
```{r, message=F,results=F, eval=FALSE, include=T}
# fuel_litter
my_fuel_litter <- list(
  ilitter = 1
  , lrho = 13.44
