...
The `cloud2trees` package relies on external data to estimate tree DBH and tree forest type. If this is functionality you will use, the data must be downloaded first using `get_data()` which only needs to be run the first time you use the `cloud2trees` package.
