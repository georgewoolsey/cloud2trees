...
- [cloud2trees: Aerial point cloud data to forest inventory tree
  lists](#cloud2trees-aerial-point-cloud-data-to-forest-inventory-tree-lists)
...
The development and functionality of the `cloud2trees` package relies
extensively on the groundbreaking work of others. We gratefully
acknowledge and encourage users to properly cite the authors and
contributors behind the following essential R packages:

- `lidR` package (<https://github.com/r-lidar/lidR>) for fundamental
  point cloud processing capabilities and routines
...
- `TreeLS` package (<https://github.com/tiagodc/TreeLS>) helps us
  extract DBH directly from the point cloud
...
A full demonstration of the install and setup of `cloud2trees` including
dependencies is available at the [Setup
demonstration](https://georgewoolsey.github.io/cloud2trees/articles/cloud2trees-setup.md)

It is recommended that users follow this full install demonstration
which includes program functionality checks after the installation steps
are completed.

# Extract Trees from Point Cloud: Default

In addition to `cloud2trees`, we’ll be using the `tidyverse`, `sf`, and
...
function is an all-in-one function to process raw .las\|.laz files to
generate a CHM raster (.tif), a DTM raster (.tif), and a tree list with
tree location, height, and DBH.

For our example we’ll use the `MixedConifer.laz` that ships with the
`lidR` package (<https://r-lidar.github.io/lidRbook/>).

A most basic example using all
[`cloud2trees()`](https://georgewoolsey.github.io/cloud2trees/reference/cloud2trees.md)
function defaults with a single .laz file and writing the output to a
...
[`cloud2trees()`](https://georgewoolsey.github.io/cloud2trees/reference/cloud2trees.md)
to perform the processing required by setting the parameters:
...
Let’s plot these tree crown polygons using
...
Let’s plot these tree top points using
[`ggplot2::ggplot()`](https://ggplot2.tidyverse.org/reference/ggplot.html)
with some custom plot settings.
...
function is used to visually assess tree crown delineation results from
different window size functions used for the detection of individual
trees.
[`itd_tuning()`](https://georgewoolsey.github.io/cloud2trees/reference/itd_tuning.md)
allows users to test different window size functions on a sample of data
to determine which function is most suitable for the area being
analyzed. The preferred function can then be used in the `ws` parameter
in
...
[`itd_tuning()`](https://georgewoolsey.github.io/cloud2trees/reference/itd_tuning.md)
enables users to sample from up to five 0.1 ha plots (`n_samples`
parameter) that are randomly selected from within the bounding box of
the point cloud data coverage. A function or a named list of functions
can be tested using the `ws_fn_list` parameter which can also be left at
the default value (`NULL`) to test default exponential (concave up),
linear, and logarithmic (concave down) functions. We’ll run
[`itd_tuning()`](https://georgewoolsey.github.io/cloud2trees/reference/itd_tuning.md)
...
run
[`itd_tuning()`](https://georgewoolsey.github.io/cloud2trees/reference/itd_tuning.md)
with our custom window size definitions and try on two sample plots of
0.1 ha
...
#> $ landfire_crown_biomass_kg <dbl> 10.08907036, 8.71581875, 0.39693953, 0.86024…
```
...
on the `estimate_tree_competition` parameter in the
[`cloud2trees()`](https://georgewoolsey.github.io/cloud2trees/reference/cloud2trees.md)
...
The
[`cloud2trees()`](https://georgewoolsey.github.io/cloud2trees/reference/cloud2trees.md)
process generates a list of outputs which are written to the disk in a
delivery directory titled *point_cloud_processing_delivery* in the
`output_dir` argument defined by the user in the function call.
...
our local machine (e.g. “C:”). Let’s check out the files that were
...
function to use the output from
[`cloud2trees()`](https://georgewoolsey.github.io/cloud2trees/reference/cloud2trees.md)
to generate inputs for the [LANL TREES](https://github.com/lanl/Trees/)
program as a pathway to fire modeling with
[Quic-Fire](https://doi.org/10.1016/j.envsoft.2019.104616)

If you want to use the
[`cloud2trees()`](https://georgewoolsey.github.io/cloud2trees/reference/cloud2trees.md)
framework to generate all of the required fire modeling input variables,
...
[`cloud2trees_to_lanl_trees()`](https://georgewoolsey.github.io/cloud2trees/reference/cloud2trees_to_lanl_trees.md)
is a directory with outputs from
[`cloud2trees()`](https://georgewoolsey.github.io/cloud2trees/reference/cloud2trees.md).
As a reminder, the default directory written by
[`cloud2trees()`](https://georgewoolsey.github.io/cloud2trees/reference/cloud2trees.md)
is *point_cloud_processing_delivery*. Also required is a spatial file
defining the boundary of our area of interest.

We’ll pretend our area of interest is the central 2,000 m² of our point
cloud data
...
Now, we’ll call the function and tell it where our
[`cloud2trees()`](https://georgewoolsey.github.io/cloud2trees/reference/cloud2trees.md)
...
[`cloud2trees_to_lanl_trees()`](https://georgewoolsey.github.io/cloud2trees/reference/cloud2trees_to_lanl_trees.md)
requires the user to specify surface fuel load parameters, such as
litter and herbaceous/grass fuel loads, which are assumed constant
across the study area. Surface fuel loading parameters should be
determined through a literature review or expert opinion.

The `fuel_litter` argument of the
[`cloud2trees_to_lanl_trees()`](https://georgewoolsey.github.io/cloud2trees/reference/cloud2trees_to_lanl_trees.md)
function allows users to define the litter fuel load with the parameters
in order:
...
The `fuel_grass` argument of the
[`cloud2trees_to_lanl_trees()`](https://georgewoolsey.github.io/cloud2trees/reference/cloud2trees_to_lanl_trees.md)
function allows users to define the grass/herbaceous fuel load with the
parameters in order:
...
function if we only want to create a DTM and CHM from our point cloud
data. This function also creates a classified and height normalized
point cloud in the process. If you wish to keep these point clouds,
ensure to turn on the `keep_intrmdt` parameter and see the
...
We can use the
[`raster2trees()`](https://georgewoolsey.github.io/cloud2trees/reference/raster2trees.md)
function if we already have a CHM raster and want to extract a tree
list.
...
[`trees_type()`](https://georgewoolsey.github.io/cloud2trees/reference/trees_type.md)
function attach the tree forest type based on the spatial overlap with
the [Forest Type Groups of the Continental United
...
`tree_y` to the
[`trees_type()`](https://georgewoolsey.github.io/cloud2trees/reference/trees_type.md)
function. We can also use an `sf` class object with POINT or POLYGON
geometry (see
[`sf::st_geometry_type()`](https://r-spatial.github.io/sf/reference/st_geometry_type.html))
and the program will use the data “as-is” and only require the `treeID`
column.
...
Use the
[`trees_type()`](https://georgewoolsey.github.io/cloud2trees/reference/trees_type.md)
function to extract the FIA forest type group based on tree location. If
a tree overlaps with an area that is classified as “non-forest”, the
program will search for the nearest forest type to impute a value; we’ll
limit the search radius by setting the `max_search_dist_m` parameter to
88 meters.
...
![](reference/figures/README-unnamed-chunk-54-1.png)
...
![](reference/figures/README-unnamed-chunk-56-1.png)
...
extract tree CBH from the point cloud using the
[`trees_cbh()`](https://georgewoolsey.github.io/cloud2trees/reference/trees_cbh.md)
function.
...
(e.g. [`cloud2raster()`](https://georgewoolsey.github.io/cloud2trees/reference/cloud2raster.md))
...
the
[`trees_hmd()`](https://georgewoolsey.github.io/cloud2trees/reference/trees_hmd.md)
function and the `estimate_tree_hmd` in the
[`cloud2trees()`](https://georgewoolsey.github.io/cloud2trees/reference/cloud2trees.md)
function may be relevant to your interests.

We just need to pass a `sf` class object with POLYGON geometry and the
columns `treeID` and `tree_height_m` and the height normalized point
cloud data to the
[`trees_hmd()`](https://georgewoolsey.github.io/cloud2trees/reference/trees_hmd.md)
function. The function returns the data with the added columns:
...
![](reference/figures/README-unnamed-chunk-63-1.png)
...
The
[`cloud2trees()`](https://georgewoolsey.github.io/cloud2trees/reference/cloud2trees.md)
package includes methods for estimating individual tree biomass in
kilograms, or the component biomass of the tree crown in kilograms.
...
function streamlines the process for estimating individual tree biomass
in kilograms, or the component biomass of the tree crown in kilograms.
Users can select one, some, or all of the methods available in the
package for estimating biomass.
...
and
[`trees_biomass()`](https://georgewoolsey.github.io/cloud2trees/reference/trees_biomass.md)
if using the Cruz or LANDFIRE methods also require the following data
attributes:
...
in kilograms, or the component biomass of the tree crown in kilograms.
Users can select one, some, or all of the methods available in the
package for estimating biomass. The following function calls are
equivalent:
...
We’ll use the
[`trees_biomass()`](https://georgewoolsey.github.io/cloud2trees/reference/trees_biomass.md)
function and ask for both the LANDFIRE and the Cruz et al. (2003)
estimates of tree crown biomass in kilograms with the argument
