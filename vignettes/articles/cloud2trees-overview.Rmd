...
cloud2trees_ans <- cloud2trees::cloud2trees(input_las_dir = las_fpath, output_dir = tempdir())
```
...
While the plot shows a fair amount of variation, let’s remember it only represents `r scales::comma(diff(terra::minmax(cloud2trees_ans$dtm_rast)),accuracy = 0.01)` m of vertical relief. If we looked at an area with 60 m of vertical relief, for example, we would not even notice these small fluctuations.
...
since we have the X and Y coordinates of those trees, we can easily load those onto a tablet or GPS device to navigate to those important trees in the field
